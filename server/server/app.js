...
const MONGO_URL =
  ...
  "mongodb+srv://admin:password123123@cluster0-m0lca.mongodb.net/books?retryWrites=true&w=majority";
...
let openPaths = [
  ...
  "/api/users/create",
  { url: "/api/categories", methods: ["GET"] } // Open GET categories, but not POST.
];

// Validate the user using authentication. checkJwt checks for auth token.
const secret = process.env.SECRET || "the cake is a lie";
if (!process.env.SECRET) console.log("Warning: SECRET is undefined.");

// Allow posts and deleting categoris only for admin
app.post("/api/categories", checkJwt({ secret: secret }), function(req, res) {
  if (req.url === "/api/categories" && !req.user.admin) {
...
// Allow posts and deleting categoris only for admin
app.delete("/api/categories", checkJwt({ secret: secret }), function(req, res) {
  if (req.url === "/api/categories" && !req.user.admin) {
    return res.sendStatus(401);
  }
  next();
...
mongoose
  ...
  .then(async () => {
    ...
    const categoryRouter = require("./routers/categories_router")(categoryDAL);
